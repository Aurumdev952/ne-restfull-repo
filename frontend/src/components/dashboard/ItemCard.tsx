...
import { DollarSign, Package, Edit, Trash2, Eye } from 'lucide-react';
  ...
  return (
    <Card className="flex flex-col bg-white shadow-lg hover:shadow-xl transition-shadow duration-300">
      <CardHeader className="pb-3">
        ...
        <CardTitle className="text-xl font-semibold text-slate-800">{item.name}</CardTitle>
      ...
      <CardContent className="flex-grow space-y-2">
        ...
        <div className="flex items-center text-sm text-slate-600">
          ...
          Price: <span className="font-semibold ml-1">${item.price.toFixed(2)}</span>
        </div>
        <p className="text-sm text-slate-600">Stock: <span className="font-semibold">{item.stock}</span></p>
      </CardContent>
      <CardFooter className="flex justify-between items-center pt-3 border-t mt-auto">
        <Button variant="outline" size="sm" asChild className="text-brand-primary border-brand-primary hover:bg-brand-primary/10">
